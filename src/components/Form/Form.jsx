...
const Form = ({ currentId, setCurrentId }) => {
  ...
  // دالة ذكية للتحقق من حقل واحد فقط أثناء الكتابة
  const validateField = async (name, value) => {
    try {
      // نتحقق من الحقل المحدد فقط داخل الـ schema
      await Yup.reach(postSchema, name).validate(value);
      setErrors((prev) => ({ ...prev, [name]: null })); // امسح الخطأ إذا كان سليماً
    } catch (yupError) {
      setErrors((prev) => ({ ...prev, [name]: yupError.message })); // سجل الخطأ الجديد
    }
  };
  ...
  const handleInputChange = (e) => {
    ...
    // تشغيل التحقق فوراً أثناء الكتابة
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentId) {
        dispatch(updatePost({ id: currentId, updatedPost: dataToSend }));
      } else {
        dispatch(createPost({ newPost: dataToSend, navigate }));
      }
      clearInput();
    } catch (yupError) {
  ...
  return (
    <Paper className={classes.paper} elevation={4} sx={{ p: 2 }}>
      <form autoComplete="off" noValidate className={classes.form} onSubmit={handleSubmit}>
        ...
        <TextField
          name="message" variant="outlined" label="Message" fullWidth multiline rows={4}
        ...
        <Button variant="outlined" color="secondary" size="small" onClick={clearInput} fullWidth>Clear</Button>
      </form>
    </Paper>
  );
};
